export default {
  ...
  commands: {
    ...
    bot: [
      ...
      "✨ Core: <c>v{13}+{14}/{15}</c>",
      "📦 Instance: <c>{16}</c>",
      "🪁 Core environment: <c>{17}</c>",
      "🧇 Client environment: <c>{18}</c>",
      "🏷 TypeScript: <c>{19}</c>",
      "🆙 Update date: <c>{20}</c> {3} <c>{21}</c>",
      "🍒 Creation date: <c>{22}</c>",
      "😼 Author: <c>{23}</c>",
      "\n📱 This bot was created <c>completely free</c> on {24}, you can also do it to use it on your groups!",
      "\n🤗 Join our <c>Discord server</c> to stay up to date with <c>Ambient</c> news, as well as get help and answers to your questions: {25}",
    ],
    group: {
