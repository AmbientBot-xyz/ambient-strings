export default {
  ...
  commands: {
    ...
    bot: [
      ...
      "📦 Instancja: <c>{16}</c>",
      "🪁 Środowisko jądra: <c>{17}</c>",
      "🧇 Środowisko klienta: <c>{18}</c>",
      "🏷 TypeScript: <c>{19}</c>",
      "🆙 Data aktualizacji: <c>{20}</c> {3} <c>{21}</c>",
      "🍒 Data powstania: <c>{22}</c>",
      "😼 Autor: <c>{23}</c>",
      "\n📱 Ten bot został stworzony <c>całkowicie za darmo</c> na stronie {24}, Ty również możesz to zrobić aby korzystać z niego na swoich grupach!",
      "\n🤗 Dołącz na nasz <c>serwer Discord</c> aby być na bieżąco z nowościami dotyczącymi <c>Ambienta</c>, a także uzyskać pomoc i odpowiedzi na pytania: {25}",
    ],
    group: {
    ...
    profile: {
      ...
      collecting: "⏳ Zbieram informacje o <c>{0}</c>...",
        ...
        "🤖 Jest botem: <c>{15}</c>",
        "👑 Globalny administrator: <c>{16}</c>",
        "👅 Język w Ambiencie: <c>{17}</c>",
        "🎛️ Połączone Spotify: <c>{18}</c>",
        "🐒 Wzmianki: <c>{19}</c> (w tej grupie: <c>{20}</c>)",
        "✉ Wysłane wiadomości: <c>{21}</c> (w tej grupie: <c>{22}</c>)",
        "🪄 Wykonane komendy: <c>{23}</c> (w tej grupie: <c>{24}</c>)",
        "🔄 Zaktualizowano dane: <c>{25}</c> {12} <c>{26}</c>",
      ],
    },
    skin: {
      usage: [
